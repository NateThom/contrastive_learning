...
class Att_Resnet(pl.LightningModule):
    def __init__(self, hparams):
        ...
        # self.resnet = resnet50(pretrained=hparams.pretrain)
        self.resnet = resnet152(pretrained=hparams.pretrain)
    ...
    def validation_step(self, val_batch, batch_idx):
        ...
        self.log("Blonde Accuracy Step", self.blonde_accuracy(attribute_preds[:,2], attribute_labels[:,2].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Blonde Precision Step", self.blonde_precision(attribute_preds[:,2], attribute_labels[:,2].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Blonde Recall Step", self.blonde_recall(attribute_preds[:,2], attribute_labels[:,2].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Blonde F1 Step", self.blonde_f1(attribute_preds[:,2], attribute_labels[:,2].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)

        self.log("Black Accuracy Step", self.black_accuracy(attribute_preds[:,1], attribute_labels[:,1].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Black Precision Step", self.black_precision(attribute_preds[:,1], attribute_labels[:,1].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Black Recall Step", self.black_recall(attribute_preds[:,1], attribute_labels[:,1].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Black F1 Step", self.black_f1(attribute_preds[:,1], attribute_labels[:,1].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)

        self.log("Brown Accuracy Step", self.brown_accuracy(attribute_preds[:,3], attribute_labels[:,3].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Brown Precision Step", self.brown_precision(attribute_preds[:,3], attribute_labels[:,3].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Brown Recall Step", self.brown_recall(attribute_preds[:,3], attribute_labels[:,3].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Brown F1 Step", self.brown_f1(attribute_preds[:,3], attribute_labels[:,3].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)

        self.log("Gray Accuracy Step", self.gray_accuracy(attribute_preds[:,4], attribute_labels[:,4].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Gray Precision Step", self.gray_precision(attribute_preds[:,4], attribute_labels[:,4].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Gray Recall Step", self.gray_recall(attribute_preds[:,4], attribute_labels[:,4].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Gray F1 Step", self.gray_f1(attribute_preds[:,4], attribute_labels[:,4].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)

        self.log("Other Accuracy Step", self.other_accuracy(attribute_preds[:,0], attribute_labels[:,0].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Other Precision Step", self.other_precision(attribute_preds[:,0], attribute_labels[:,0].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Other Recall Step", self.other_recall(attribute_preds[:,0], attribute_labels[:,0].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Other F1 Step", self.other_f1(attribute_preds[:,0], attribute_labels[:,0].int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
    ...
    def test_step(self, test_batch, batch_idx):
        ...
        self.log("Test Accuracy Step", self.valid_accuracy(attribute_preds, attribute_labels.int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Test Precision Step", self.valid_precision(attribute_preds, attribute_labels.int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Test Recall Step", self.valid_recall(attribute_preds, attribute_labels.int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
        self.log("Test F1 Step", self.valid_f1(attribute_preds, attribute_labels.int()), on_step=True, on_epoch=False, sync_dist=True, logger=True)
    ...
    def test_epoch_end(self, outputs):
        self.log("Test Accuracy Epoch", self.valid_accuracy.compute(), on_step=False, on_epoch=True, sync_dist=True, logger=True)
        self.log("Test Precision Epoch", self.valid_precision.compute(), on_step=False, on_epoch=True, sync_dist=True, logger=True)
        self.log("Test Recall Epoch", self.valid_recall.compute(), on_step=False, on_epoch=True, sync_dist=True, logger=True)
