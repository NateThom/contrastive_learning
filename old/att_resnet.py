...
class Att_Resnet(pl.LightningModule):
    def __init__(self, hparams):
        ...
        if hparams.simclr == True:
            dim_mlp = self.net.fc.out_features
            self.net.fc = nn.Sequential(self.net.fc, nn.ReLU(), nn.Linear(dim_mlp, 128))
    ...
    def training_step(self, train_batch, batch_idx):
        ...
        attribute_preds = self.net(inputs)
        ...
        if self.simclr == False:
            ...
            inputs2 = train_batch["image2"]
            attribute_preds2 = self.net(inputs2)
            attribute_preds3 = torch.cat((attribute_preds, attribute_preds2), dim=0)
            logits, labels = self.info_nce_loss(attribute_preds3)
    ...
    def configure_optimizers(self):
        ...
        lr_scheduler = optim.lr_scheduler.CosineAnnealingLR(optimizer, T_max=self.train_size/self.batch_size)
